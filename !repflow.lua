...
local changelogEntries = {
    { version = "4.12 | Premium", description = "- Добавлен тестер Sora_Deathmarried с пометкой (лох) в благодарности." },
...
function drawUpdatesTab()
    ...
    if imgui.BeginChild("Updates", imgui.ImVec2(0,120), true) then
        ...
        imgui.Separator()
        imgui.TextDisabled(u8"Обновление скачивается напрямую с GitHub")
    end
    imgui.EndChild()
    imgui.PopStyleColor()
end
...
function drawInfoTab(panelColor)
    ...
    if imgui.BeginChild("Info3", imgui.ImVec2(0,140), true) then
        ...
        imgui.Text(u8'Тестер: Sora_Deathmarried (лох)')
    end
    imgui.EndChild()
    imgui.PopStyleColor()
end
